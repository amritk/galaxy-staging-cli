# Demo API (Scalar Galaxy) CLI API
...
```sh
demoapiscalargalaxy planets create \
...
```sh
demoapiscalargalaxy planets retrieve '1' --bearer-auth "$BEARER_AUTH"
```
...
```sh
demoapiscalargalaxy planets update '1' \
  --bearer-auth "$BEARER_AUTH" \
  --name 'Mars' \
  --type 'terrestrial'
```
...
```sh
demoapiscalargalaxy planets delete '1' --bearer-auth "$BEARER_AUTH"
```
...
```sh
demoapiscalargalaxy planets upload-image '1' --bearer-auth "$BEARER_AUTH"
```
...
```sh
demoapiscalargalaxy celestial-bodies create --bearer-auth "$BEARER_AUTH"
```
...
```sh
demoapiscalargalaxy authentication create-user \
  --bearer-auth "$BEARER_AUTH" \
  --name 'Marc' \
...
```sh
demoapiscalargalaxy authentication create-token \
  --bearer-auth "$BEARER_AUTH" \
  --email 'marc@scalar.com' \
  --password 'i-love-scalar'
```
...
```sh
demoapiscalargalaxy authentication list-me --bearer-auth "$BEARER_AUTH"
```
